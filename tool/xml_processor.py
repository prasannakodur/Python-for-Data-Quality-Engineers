...
class XMLRecordProcessor:
    ...
    def process_record(self, record_dict: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if record_type == "news":
                formatted_record = create_news(record_dict["text"], record_dict["city"])
                # Save to database if enabled
                if self.db_manager:
                    success, msg = self.db_manager.save_news(record_dict["text"], record_dict["city"])
                    print(f"    DB: {msg}")
            
            elif record_type in ("privatead", "ad"):
                formatted_record = create_private_ad(record_dict["text"], record_dict["expiration"])
                # Save to database if enabled
                if self.db_manager:
                    success, msg = self.db_manager.save_private_ad(record_dict["text"], record_dict["expiration"])
                    print(f"    DB: {msg}")
            
            elif record_type == "recipe":
                formatted_record = create_recipe(record_dict["title"], record_dict["ingredients"])
                # Save to database if enabled
                if self.db_manager:
                    # Parse ingredients to get count and complexity
                    import re
                    ingredients_list = [i.strip() for i in re.split(r'[,;]', record_dict["ingredients"]) if i.strip()]
                    ...
                    success, msg = self.db_manager.save_recipe(
                        record_dict["title"], record_dict["ingredients"],
